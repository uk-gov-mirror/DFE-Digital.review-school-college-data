...
namespace Web09.Checking.Business.Logic.TransactionScripts
{
    ...
    {
        public static List<SchoolDocumentsInformation> GetSchoolDocuments(int? DFESNumber)
        {
            try
            {
                using (EntityConnection conn = new EntityConnection("name=Web09_Entities"))
                {
                    ...
                    using (Web09_Entities context = new Web09_Entities(conn))
                    {
                        ...
                        cmd.CommandText = "School.GetDocumentListBySchool";
                        ...
                        while (dr.Read())
                        {
                            ...
                            SchoolDocumentsInformation ety = new SchoolDocumentsInformation
                            {
                                ...
                                HideLinkIfNoDocument = dr.IsDBNull(dr.GetOrdinal("HideLinkIfNoDocument")) ? false : Convert.ToBoolean(dr["HideLinkIfNoDocument"].ToString()),
                                LocationIsUnderStaticDataPath = dr.IsDBNull(dr.GetOrdinal("LocationIsUnderStaticDataPath")) ? true : Convert.ToBoolean(dr["LocationIsUnderStaticDataPath"].ToString())
                            };
