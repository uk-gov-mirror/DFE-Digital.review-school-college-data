    ...
    public class CrmAmendmentService : IAmendmentService
    {
        ...
        public string CreateAmendment(Amendment amendment)
        {
            using (var context = new CrmServiceContext(_organizationService))
            {
                ...
                _organizationService.Associate(rscd_Amendment.EntityLogicalName, amendmentDto.Id, relationship, new EntityReferenceCollection
                {
        ...
        public void RelateEstablishmentToAmendment(cr3d5_establishment establishment, Guid amendmentId, CrmServiceContext context)
        {
        ...
        public IEnumerable<Amendment> GetAmendments(CheckingWindow checkingWindow, string urn)
        {
            using (var context = new CrmServiceContext(_organizationService))
            {
                ...
                var amendments = context.rscd_AmendmentSet.Where(
                                     x => x.rscd_Establishmentv2.Id == establishment.First().Id && x.rscd_Checkingwindow == rscd_Checkingwindow.KS5).ToList();
