...
namespace Dfe.CspdAlpha.Web.Infrastructure.SharePoint
{
    ...
    /// It is a modified version of the Microsoft original https://docs.microsoft.com/en-us/sharepoint/dev/sp-add-ins/using-csom-for-dotnet-standard
    /// Modified to use client id/secret rather than username/password.
    /// This class can be removed when we can use 
    /// TODO: Remove this class when PnP Core supports .NET Standard (https://github.com/pnp/PnP-Sites-Core)
    /// </summary>
    public class AuthenticationManager : IDisposable
    {
        ...
        public AuthenticationManager(string tenantId)
        {
            _tenantId = tenantId;
        }
                ...
                try
                {
                    ...
                    wi.Handle = ThreadPool.RegisterWaitForSingleObject(
                        ...
                        async (state, timedOut) =>
                        {
                            if (!timedOut)
                            {
                                TokenWaitInfo twi = (TokenWaitInfo)state;
                                if (twi.Handle != null)
                                {
                                    twi.Handle.Unregister(null);
                                }
                            }
                            else
                            {
        ...
        private async Task<string> AcquireTokenAsync(Uri web, string clientId, string clientSecret)
        {
            var body = "grant_type=client_credentials" +
                $"&resource=00000003-0000-0ff1-ce00-000000000000/{web.DnsSafeHost}@{_tenantId}" +
                $"&client_id={clientId}@{_tenantId}" +
                $"&client_secret={clientSecret}";

            using (var stringContent = new StringContent(body, Encoding.UTF8, "application/x-www-form-urlencoded"))
            {
                var result = await httpClient.PostAsync($"https://accounts.accesscontrol.windows.net/{_tenantId}/tokens/OAuth/2", stringContent)
